...
mod real_api_tests {
    ...
    async fn test_real_basic_message() {
        ...
        assert!(
            response.text().contains("Hello from Threatflux"),
    ...
    async fn test_real_streaming_message() {
        ...
        assert!(
            text.is_ok(),
            "Stream collection should succeed: {:?}",
            text.err()
        );
        ...
        for i in 1..=5 {
            assert!(
                collected_text.contains(&i.to_string()),
    ...
    async fn test_real_token_counting() {
        ...
        assert!(
            ...
            "Should have positive token count, got: {}",
            count.input_tokens
        );
        assert!(
            count.input_tokens < 50,
            "Token count should be reasonable for short message, got: {}",
            count.input_tokens
        );
    }
    ...
    async fn test_real_system_message() {
        ...
        assert!(
            word_count <= 5,
            "Should respond with roughly 3 words due to system prompt, got {} words: '{}'",
            word_count,
            response.text()
        );
    }
    ...
    async fn test_real_temperature_variations() {
        ...
        assert!(
            response_low.is_ok(),
            "Low temperature message should succeed"
        );
        ...
        assert!(
            response_high.is_ok(),
            "High temperature message should succeed"
        );
    ...
    async fn test_real_error_handling() {
        ...
        if let Err(error) = response {
            assert!(
                !error.is_retryable(),
                "Invalid model error should not be retryable"
            );
        }
    }
    ...
    async fn test_real_stop_sequences() {
        ...
        if text.contains("STOP") {
            ...
            assert!(
                after_stop.is_empty() || after_stop.len() < 10,
                "Should stop generation at STOP sequence, but got: '{}'",
                text
            );
        }
    }
    ...
    async fn test_real_metadata() {
        ...
        assert!(
            response.is_ok(),
            "Message with metadata should succeed: {:?}",
            response.err()
        );
        ...
        assert!(
            ...
            "Should get response with metadata"
        );
    }
}
