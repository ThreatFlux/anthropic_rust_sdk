...
mod error_tests {
    use super::*;
    // Direct (non-glob) import so this takes precedence over both the std
    // prelude `assert_eq!` macro and the `use super::*` glob, resolving the
    // ambiguity error.
    ...
    fn test_error_specific_status_codes() {
        ...
        assert!(
            !AnthropicError::api_error(413, "Payload too large".to_string(), None).is_retryable()
        );
    }
