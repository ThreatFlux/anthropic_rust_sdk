...
mod http_utils_tests {
    ...
    fn test_query_params_encoding() {
        ...
        // `query_pairs_mut` uses application/x-www-form-urlencoded serialization,
        // which encodes spaces as `+` rather than `%20`.
        assert!(query.contains("special=value+with+spaces"));
    }

    #[test]
    fn test_content_type_detection() {
        assert_eq!(
            mime_guess::from_path("test.json").first_or_octet_stream(),
            "application/json"
        );
        assert_eq!(
            mime_guess::from_path("test.txt").first_or_octet_stream(),
            "text/plain"
        );
        assert_eq!(
            mime_guess::from_path("test.png").first_or_octet_stream(),
            "image/png"
        );
        assert_eq!(
            mime_guess::from_path("test.jpeg").first_or_octet_stream(),
            "image/jpeg"
        );
    }
...
mod rate_limiting_tests {
    ...
    fn test_request_priority() {
        ...
        // RequestPriority is not Ord, so rank the variants explicitly to
        // verify the intended ordering (Low < Normal < High).
        let rank = |p: RequestPriority| match p {
            RequestPriority::Low => 0u8,
            RequestPriority::Normal => 1u8,
            RequestPriority::High => 2u8,
        };

        assert!(rank(high_priority) > rank(normal_priority));
        assert!(rank(normal_priority) > rank(low_priority));
    ...
    fn test_request_options() {
        ...
        assert_eq!(
            options.headers.get("x-custom").map(String::as_str),
            Some("value")
        );
...
mod time_handling_tests {
    ...
    fn test_timeout_calculation() {
        ...
        // Test if timeout has elapsed (should not have, immediately)
...
mod enhanced_utils_tests {
    ...
    fn test_rate_limit_info_approaching_limit() {
        ...
        // 20 remaining of 100 is 80% usage; usage_ratio (0.8) meets the 0.8
        // threshold, so this counts as approaching the limit.
        assert!(rate_limit_info.is_approaching_limit(0.8));

        // With a higher threshold, 80% usage is below it and is not approaching.
        assert!(!rate_limit_info.is_approaching_limit(0.85));
    ...
    fn test_rate_limit_config_creation() {
        let config = RateLimitConfig::new(60, Duration::from_secs(60)).with_burst(10);
