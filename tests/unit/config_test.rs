...
mod config_tests {
    ...
    fn test_config_new_valid_key() {
        ...
        // `Url::parse` normalizes a host-only URL by appending the root path,
        // so the stored base URL carries a trailing slash.
    ...
    fn test_config_new_whitespace_key() {
        // `Config::new` rejects only empty (zero-length) keys; it does not trim,
        // so a whitespace-only key is currently accepted as a non-empty value.
        let result = Config::new("   ");
        assert!(result.is_ok());
    ...
    fn test_config_builder_pattern() {
        ...
        // Host-only URLs are normalized with a trailing slash by `Url::parse`.
        assert_eq!(config.base_url.as_str(), "https://custom.api.com/");
    }

    #[test]
    fn test_config_from_env_complete() {
        let _env = super::super::env_guard();
    ...
    fn test_config_edge_cases() {
        ...
        let config = Config::new("test-key").unwrap().with_max_retries(10);
    ...
    fn test_config_partial_env() {
        ...
        // Default host-only URL is normalized with a trailing slash by `Url::parse`.
    ...
    fn test_config_default() {
        ...
        // Host-only URL is normalized with a trailing slash by `Url::parse`.
        assert_eq!(config.base_url.as_str(), "https://api.anthropic.com/");
        assert_eq!(config.timeout, Duration::from_secs(60));
        assert_eq!(config.max_retries, 3);
        assert_eq!(config.default_model, "claude-sonnet-4-6");
    ...
    fn test_config_with_admin_key() {
        let config = Config::new("test-key").unwrap().with_admin_key("admin-key");
...
mod model_tests {
    ...
    fn test_supports_1m_context() {
        ...
        assert!(!models::supports_1m_context(models::HAIKU_4_5));
    ...
    fn test_max_thinking_tokens() {
        // budget_tokens is removed for adaptive-thinking models; the helper now
        // returns None across the board (effort param supersedes token budgets).
    ...
    fn test_all_models() {
        ...
        assert!(all_models.contains(&models::SONNET_4_6));
        assert!(all_models.contains(&models::HAIKU_4_5));
    ...
    fn test_is_valid_model() {
        ...
        assert!(models::is_valid_model(models::SONNET_4_6));
        assert!(models::is_valid_model(models::HAIKU_4_5));
    ...
    fn test_model_constants() {
        // Verify current model constants are properly formatted
        assert!(models::OPUS_4_1.starts_with("claude-opus-4-1"));
        assert!(models::OPUS_4_8.starts_with("claude-opus-4-8"));
        assert!(models::OPUS_4_7.starts_with("claude-opus-4-7"));
        assert!(models::OPUS_4_6.starts_with("claude-opus-4-6"));
        assert!(models::SONNET_4_6.starts_with("claude-sonnet-4-6"));
        assert!(models::HAIKU_4_5.starts_with("claude-haiku-4-5"));
        assert!(models::FABLE_5.starts_with("claude-fable-5"));
        assert!(models::MYTHOS_5.starts_with("claude-mythos-5"));

        // Current catalog ids are all `claude-`-prefixed family/version slugs.
        for model in models::all_models() {
            assert!(
                model.starts_with("claude-"),
                "Model {} should be claude-prefixed",
                model
            );
            assert!(
                model.matches('-').count() >= 2,
                "Model {} should have a version slug",
                model
            );
        }
    }
