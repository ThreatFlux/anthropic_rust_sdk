...
mod client_tests {
    ...
    fn test_client_api_getters() {
        ...
        // Admin API requires a separate admin key; without one configured,
        // `admin()` returns an error rather than an `AdminApi` handle.
    ...
    #[test]
    fn test_client_does_not_leak_api_key() {
        let config = Config::new("test-key").unwrap();
        let client = Client::new(config);

        // For security, `Client` intentionally does not implement `Debug`, so the
        // API key can never be exposed through `{:?}` formatting. The key is still
        // accessible through the explicit config accessor for legitimate use.
        assert_eq!(client.config().api_key, "test-key");
    }
    ...
    fn test_request_options_with_beta_features() {
        let options = RequestOptions {
            ...
            beta_features: vec!["custom-feature".to_string()],
            ..Default::default()
        };
