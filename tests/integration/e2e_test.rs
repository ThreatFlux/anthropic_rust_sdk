...
mod e2e_tests {
    ...
    async fn test_complete_message_workflow() {
        ...
        Mock::given(method("POST"))
            ...
            .respond_with(ResponseTemplate::new(200).set_body_json(json!({"input_tokens": 15})))
            .mount(&mock_server)
            .await;
    ...
    async fn test_batch_processing_workflow() {
        ...
        // Mock results retrieval. The results endpoint returns JSONL: one
        // MessageBatchResultEntry per line, not a wrapped `{ "results": [...] }`.
        let batch_results = [
            json!({
                "custom_id": "request1",
                "result": {
        ...
        Mock::given(method("GET"))
            ...
            .respond_with(ResponseTemplate::new(200).set_body_string(batch_results))
            .mount(&mock_server)
            .await;
        ...
        // Get results (returns a Vec<MessageBatchResultEntry> directly)
        let results = client
            .message_batches()
            .results(&batch.id, None)
            .await
            .unwrap();
    ...
    async fn test_file_upload_and_usage_workflow() {
        ...
        Mock::given(method("POST"))
            .and(path("/v1/messages"))
            .respond_with(ResponseTemplate::new(200).set_body_json(&message_with_file))
            .mount(&mock_server)
            .await;
        ...
        let request = MessageBuilder::new()
            ...
            .max_tokens(100)
            .user(format!(
                "Please analyze the uploaded file with ID: {}",
                file_id
            ))
            .build();
    ...
    async fn test_error_recovery_workflow() {
        ...
        // First request fails with rate limit. Higher priority + a single-match
        // cap means this mock answers the first request only; the retry then
        // falls through to the success mock mounted below.
        Mock::given(method("POST"))
            .and(path("/v1/messages"))
    ...
    async fn test_multi_modal_conversation() {
        ...
        Mock::given(method("POST"))
            .and(path("/v1/messages"))
            .respond_with(ResponseTemplate::new(200).set_body_json(&vision_response))
            .mount(&mock_server)
            .await;
        ...
        let request = MessageBuilder::new()
            .model("claude-3-5-haiku-20241022")
            .max_tokens(100)
            .user_with_base64_image("What do you see in this image?", image_data, "image/png")
            .build();
    ...
    async fn test_tool_usage_workflow() {
        ...
        // The initial turn returns the tool_use response. Cap it at one match
        // (and give it priority) so the follow-up turn falls through to the
        // final text response mounted below.
        Mock::given(method("POST"))
            .and(path("/v1/messages"))
